...
void EngineModel::shift_gear(int new_gear) {
    ...
    if (new_gear == gear_ && shift_phase_ == ShiftPhase_Idle) return;
    if (shift_phase_ != ShiftPhase_Idle) return;
    ...
    shift_phase_        = ShiftPhase_ClutchDisengage;
    shift_timer_        = 0.0f;
...
void EngineModel::set_clutch(float value) {
    ...
    if (shift_phase_ == ShiftPhase_Idle) {
...
void EngineModel::update_shift(float dt) {
    ...
    case ShiftPhase_ClutchDisengage: {
        ...
        if (shift_timer_ >= kClutchDisengageTime) {
            ...
            shift_timer_ = 0.0f;
            shift_phase_ = ShiftPhase_GearChange;
        }
        break;
    }

    case ShiftPhase_GearChange: {
        ...
        if (shift_timer_ >= kGearChangeTime) {
            ...
            blip_amount_ = 0.0f;
            shift_timer_ = 0.0f;
            shift_phase_ = ShiftPhase_ClutchEngage;
        }
        break;
    }

    case ShiftPhase_ClutchEngage: {
        ...
        if (shift_timer_ >= kClutchEngageTime) {
            clutch_ = 1.0f;
            shift_phase_ = ShiftPhase_Idle;
            shift_timer_ = 0.0f;
        }
        break;
    }

    case ShiftPhase_Idle:
        break;
    }
}
...
void EngineModel::update(float dt) {
    ...
    if (shift_phase_ == ShiftPhase_GearChange && shift_is_downshift_ && blip_amount_ > 0.0f) {
        float blip_progress = std::min(shift_timer_ / kBlipDecayTime, 1.0f);
    ...
    if (shift_phase_ != ShiftPhase_GearChange) {
