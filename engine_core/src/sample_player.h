...
public:
    ...
    // Load a sample from raw PCM float data (call during init, NOT real-time safe)
    // Returns sample_id on success, -1 on failure
    int load_sample(const float* pcm_data, uint32_t num_frames,
                    SampleRate original_rate, bool loop = false);
